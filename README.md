...
## Escenarios L2 y Persona L3 (hitos 4.1–5.5)

La capa **L2** (`apps/launcher/src/l2/`) agrupa hechos L1 en **escenarios situacionales** con clustering semántico real (embeddings de los hechos, grupos de 3+ crean nodos con nombre LLM/fallback). El panel *Memory > L2 Scenarios* muestra el grafo en un **canvas Mermaid interactivo** (SVG propio: pan/zoom, detalle al click, edición de labels, export PNG; string Mermaid copiable/descargable), incluye un **Tool Call Simulator** que comprime secuencias JSON a Mermaid con ahorro de tokens y colores por HTTP (2xx/4xx/5xx), y **drill-down L2 → L0** que recupera el entry de buffer exacto (con aviso si expiró por TTL).
